...
///////////////////////////////////////////////////////////////////////////////
bool ProbablePairRR::parse( int argc, char** argv ) {

	// Create the command line parser and build in its required parameters.
	ParseCommandLine* parser = new ParseCommandLine( "ProbablePairRR" );
...
void ProbablePairRR::run() {
	...
	if( error == 0 ) {
		...
		cout << "Writing output RR file..." << flush;

        sort(RR_list.begin(), RR_list.end(), compareRR);
        FILE* fp=fopen(ctFile.c_str(),"w");
        for (size_t l=0; l<RR_list.size(); l++)
            fprintf(fp,"%d\t%d\t%1.5f\n", RR_list[l].second.first,
                RR_list[l].second.second, RR_list[l].first);
        fclose(fp);
...
int main( int argc, char* argv[] ) {
	...
	ProbablePairRR* runner = new ProbablePairRR();
